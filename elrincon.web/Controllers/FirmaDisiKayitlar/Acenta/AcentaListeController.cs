...
{
    public class AcentaListeController : AcentaBaseController
    {
        // GET: AcentaListe
        public ActionResult Index()
        {
            AcentaListeModel model = new AcentaListeModel();
            model.GridModel = FillGridModel(FillGridCustomProperties());
            model.AcentaEditorModel = FillEditorModel();

            return View("~/Views/FirmaDisiKayitlar/Acenta/AcentaListe.cshtml", model);
        }

        private AcentaEditorModel FillEditorModel()
        {
        ...
        private ElDataGridModel FillGridModel(CustomProperties customProperties)
        {
            ...
            ElDataGridModel dataGridModel = new ElDataGridModel("acenta_liste_grid_id");
            ...
            row = new GridRow("Şehir", "sehir", "25%", InputDataType.Text);
            row.IsSortuble = true;
            dataGridModel.GridRows.Add(row);
            ...
            ActionButton buttonEdit = new ActionButton("edit", @Url.Content("~/Content/images/edit.png"), "edit_acenta_listesi_cliced");
            ActionButton buttonDelete = new ActionButton("edit", @Url.Content("~/Content/images/delete.png"), "delete_acenta_listesi_cliced");
        ...
        {
            string sql = "select a.*, l.deger as subesi_adi, l.deger as ulkesi " +
                         "from el_acenta a " +
                         "left join el_liste_deger l on l.kod = a.subesi and l.liste_id = 2 ";
                         //"left join el_liste_deger l2 on l2.kod = a.ulke and l.liste_id = 2 ";
            //" where acenta_tipi = 1";
        ...
        public JsonResult DeleteRow(int id)
        {
            ...
            try
            {
                ElDbTable db = GetTable("el_acenta");
                db.Delete("id", id); 
            }
            catch (Exception e)
            {
